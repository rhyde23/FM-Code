...
for team in teams_in_league :
    path = 'C:\\Users\\rhyde23\\Desktop\\Project\\Images\\'+team+'.png'
    if pi :
        path = convert_path(path)
    team_logos[team] = pygame.transform.scale(pygame.image.load(path).convert_alpha(), (40, 40))
...
while manager_loop :
    while calendar :
        ...
        for game_date in dates_of_games_in_month :
            opponent = dates_of_games_in_month[game_date]
            m, d = game_date.split(' ')
        ...
        display_button(calendar_buttons[0], pygame.mouse.get_pos())
        pygame.display.update()
    while saves_menu :
        display.fill(white)
